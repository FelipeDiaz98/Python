...
cont = 0    # para contar el numero de taus que se cuentan mal como jets
...
for n_signal, signal in enumerate(signals):
	...
	for ind in range(1, jobs[n_signal] + 1):
		...
		directory = str(signal + "/" + signal + "_" + str(ind) + "/Events/run_01/tag_1_delphes_events.root")
		...
		for i in range(Number):
			...
			if (len(jets) >= 4 and len(bs) >= 2 and len(taus) >= 2):
				...
				taus.sort(reverse = True, key=PT_taus)

				j1, j2, j3, j4, b1, b2, tau1, tau2 = event_reconstruction(jets, bs, taus)

				#Variables to be plotted.
				variables = [b1.Pt(), b1.Eta(), b1.Phi(), b2.Pt(), b2.Eta(), b2.Phi(), tau1[0].Pt(), tau1[0].Eta(), tau1[0].Phi(), tau2[0].Pt(), tau2[0].Eta(), tau2[0].Phi(), j1.Pt(), j1.Eta(), j1.Phi(), j2.Pt(), j2.Eta(), j2.Phi(), j3.Pt(), j3.Eta(), j3.Phi(), j4.Pt(), j4.Eta(), j4.Phi(), tau1[0].DeltaR(tau2[0]), j1.DeltaR(j2), j1.DeltaR(j3), j1.DeltaR(j4), j2.DeltaR(j3), j2.DeltaR(j4), j3.DeltaR(j4), tau1[0].DeltaPhi(tau2[0]), j1.DeltaPhi(j2), j1.DeltaPhi(j3), j1.DeltaPhi(j4), j2.DeltaPhi(j3), j2.DeltaPhi(j4), j3.DeltaPhi(j4), (tau1 + tau2).M() + Total_MET, (j1+j2).M(), (j1+j3).M(), (j1+j4).M(), (j2+j3).M(), (j2+j4).M(), (j3+j4).M(), j1.Pt() + j2.Pt() + j3.Pt() + j4.Pt() + b1.Pt() + b2.Pt() + tau1.Pt() + tau2.Pt() + Total_MET]
